(function () {
	...
	document.querySelectorAll('[data-feedback-form]').forEach(function (form) {
		...
		function updateScope() {
			sectionField.hidden = scope.value !== 'section';
		}
		function saveDraft() {
			window.clearTimeout(timer);
			timer = window.setTimeout(function () {
				try {
					window.localStorage.setItem(key, JSON.stringify({ notes: notes.value, scope: scope.value }));
					saved.textContent = 'Szkic uwag zapisany lokalnie.';
				} catch (ignore) {}
			}, 250);
		}
		updateScope();
		scope.addEventListener('change', function () { updateScope(); saveDraft(); });
