...
foreach (['lead', 'fact-1', 'takeaway'] as $anchor) {
    $visualPlan['inline_slots'][] = ['slot_id'=>'inline-' . $anchor,'role'=>'inline','section_anchor'=>$anchor,'topic_source'=>'A','visual_need'=>'Direct inline subject','must_be_direct'=>false,'acceptable_related'=>true,'search_queries_direct'=>['inline subject'],'search_queries_related'=>['related context'],'required'=>true];
}
...
{
    $db->prepare('INSERT INTO article_images (post_id, role, section_id, visual_intent, expected_content, search_queries_json, source_page_url, source_file_url, local_path, author, license, license_url, attribution, alt, caption, layout, status, relationship, is_fallback, editorial_rejected, multimodal_accepted, multimodal_assessment_json) VALUES (:post,:role,:section,"test","test","[]",:source,"https://example.test/file.jpg","images/posts/p4-coverage-test.jpg","Test","cc0","https://example.test/license","Test","alt","caption","full","downloaded",:relationship,:fallback,0,1,:assessment)')->execute([':post'=>$postId, ':role'=>$role, ':section'=>$section, ':relationship'=>$relationship, ':fallback'=>$fallback, ':source'=>$relatedSupported ? 'https://example.test/context' : 'https://example.test/source', ':assessment'=>json_encode(['related_supported'=>$relatedSupported], JSON_THROW_ON_ERROR)]);
}
