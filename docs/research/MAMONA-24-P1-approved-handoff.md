...
## Final P1 Architecture

P1_ARCHITECTURE_RESULT — P1-C KOREKTA

### Status

COMPLETE — P1-C korekta, 2026-08-06. Model: qwen3.6:27b/deep.
...
Maksimum: 20 odpowiedzi. Nowa tabela article_generation_budget + zmiana thresholdu w gemini-quota-service.php.
...
Twarde bramki: char_count_range, gemini_budget_limit, max_5_images, required_slots_filled, assets_exist, rights_license_ok, metadata_consistent, publication_safe, no_fallback_images (nowa).
Miękkie bramki: narrative_coherence, transitions_smooth, no_redundancy, rhythm_varied, engagement_level, not_monotonic_matrix.
Dodano convergence_check: boolean — w convergence mode nie obniża się thresholdów.
...
informational: min 3000, max 7000 (+2000).
problem_discovery_return: min 4000, max 7000 (+2000).

### Kanoniczne liczenie znaków

mb_strlen(implode(...)) z article_draft_main_content_length(). Ta sama funkcja w generatorze, QC, logach i testach.

### Kanoniczne liczenie grafik

ceil(target_length / 1200), max 5, min 1. Hero wliczony. Zamrożone po zaakceptowaniu NarrativePlan.
...
Kryteria detekji (co najmniej jeden):
...
Dry-run manifest, backup z SHA-256, rollback SQL: UPDATE article_images SET is_fallback = 0 WHERE id IN (lista_z_backupu).
...
### Dry-run, manifest, backup i checksum

Manifest: candidates[] z article_id, title_seed, qualification_reason, current_status, is_public, assets[], fields_to_clean[], fields_to_preserve[].
Backup: JSON eksport dotkniętych rekordów + SHA-256 w logu.
...
Walidacja: sprawdź kolumny, tabele, przelicz fallbacki, weryfikuj checksum.

### Zgodność wsteczna

(1) is_fallback default 0 + backfill dla starych fallbacków. (2) Budget table — brak wpisu = brak budżetu (tylko nowe przebiegi). (3) NarrativePlan — brak rekordu = stara ścieżka. (4) quality_check_runs — nowa bramka no_fallback_images dodatkowa. (5) Limit 15→20 atomowy w kodzie, bez okna z dwoma limitami.

### Pliki i symbole wymagające późniejszej zmiany
...
O2: Dokładna definicja "artefakty pochodne" w resecie (F9) — konkretny SQL DELETE/UPDATE dla każdej tabeli. Faza P2-G.
O3: NarrativePlan i generation_batch_items.stage (F11) — nowy stage vs. pod-etap research/draft. Faza P2-B.
O4: Kalibracja thresholdu bramki semantycznej (F12) — domyślny 60, walidacja na fixture'ach. Faza P2-D.
O5: Polityka expiracji gemini_call_cache — cache bez TTL może zwracać przestarzałe odpowiedzi. Faza P2-A.

### Braki

12 nierozstrzygniętych pytań z P0 pozostają otwarte i będą rozwiązywane w trakcie P2:
1. Czy istnieją inne typy tekstów? 2. Limity lead.text, why_important.text, key_facts[*].text? 3. build_generation_prompt() — reguły długości w prompcie czy tylko w schemacie? 4. Implementacja quality_check_auto_repair_decision()? 5. Implementacja promote_article_draft_to_post()? 6. Logowanie wywołań Gemini poza gemini_quota_events? 7. Szczegóły generation_batch_worker.php? 8. Cache gemini_call_cache — TTL/expiracja? 9. Definicja stałej QUALITY_PASS_SCORE? 10. Pełna funkcja quality_check_schema()? 11. render_article_image_record($image, true) dla hero? 12. validate_article_blocks()? 13. Dedykowana funkcja resetu wadliwego artykułu? 14. Warunki assert_post_quality_allows_publication()? 15. post_legacy_publication_flag() → is_published?
...
F2 — Konflikt budżetu 15 vs 20 (CRITICAL) → ROZWIĄZANE
Poprawiony raport zawiera dedykowaną sekcję "Migracja limitu 15 do 20" z czterema jawnymi krokami. Stary limit jest zastępowany, nie pozostawiany równolegle. Decyzja A10 precyzyjna. Tabela "Co zużywa budżet, co nie" precyzuje każdy typ wywołania.

F4 — Salvage fallback status='downloaded' (CRITICAL) → ROZWIĄZANE
Sekcja "Backfill starych fallbacków" z trzema deterministycznymi kryteriami detekji, backfillem SQL w tej samej transakcji co ADD COLUMN, dry-run manifestem, backupiem SHA-256 i rollbackiem SQL.

F3 — Convergence mode propagation (HIGH) → ROZWIĄZANE
Dwa miejsca przechowywania: generation_batch_items.convergence_active (kolumna BOOLEAN DEFAULT 0) + parametr przekazywany do funkcji. Automatyczne ustawianie po inkrementacji ≥ 16. Odczyt przez downstream: repair_router_assess, quality_check_auto_repair_decision, draft repair generator.

F6 — Publication gate nie sprawdza grafik (HIGH) → ROZWIĄZANE
Dwa warunki dodane PRZED sprawdzaniem QC score: brak fallbacków + minimalna liczba grafik. QcReport otrzymał nową twarde brame no_fallback_images.

F10 — Brak testu interakcji budżetów 15→20 (HIGH) → ROZWIĄZANE
Test T19 w macierzy: call 16-20 nie rzucają GeminiTopicBudgetException; call 21 rzuca GeminiArticleBudgetException.

F7 — Mock transport i budżet (MEDIUM) → OPEN ISSUE O1
F8 — Backfill is_fallback (MEDIUM) → ROZWIĄZANE przez F4
F9 — Reset: definicja artefaktów pochodnych (MEDIUM) → OPEN ISSUE O2
F11 — NarrativePlan i batch stage (MEDIUM) → OPEN ISSUE O3
F12 — Semantic gate threshold (MEDIUM) → OPEN ISSUE O4

### Weryfikacja centralnego limitu 20

Jeden centralny limit dokładnie 20: TAK — GeminiBudget.max_calls: 20; tabela article_generation_budget.
Stary limit 15 zastąpiony, nie równoległy: TAK — A10, Krok 1-4 migracji.
Convergence mode od pozycji 16: TAK — convergence_threshold: 16; automatyczne ustawianie flagi.
Po pozycji 20 żadne wywołanie nie dozwolone: TAK — GeminiArticleBudgetException → manual_review.
Retry transport nie zużywa budżetu: TAK — tabela potwierdza NIE.
Contract repair, research retry, QC auto-repair zużywają budżet: TAK — tabela potwierdza TAK.
Mock transport w testach nie zużywa budżetu: TAK — gemini_mock_budget_bypass; open issue O1.

### Weryfikacja fallbacków

Stare fallbacki rozpoznane deterministycznie: TAK — 3 kryteria.
Backfill w tej samej transakcji co ADD COLUMN: TAK — Krok 2d migracji SQLite.
Brak okna czasowego: TAK — transakcyjny backfill przed COMMIT.
Dry-run manifest: TAK — sekcja "Backfill starych fallbacków".
Backup z SHA-256: TAK.
Rollback backfillu: TAK — SQL UPDATE is_fallback=0.

### Weryfikacja migracji SQLite

Backup przed migracją: TAK — Krok 1.
Transakcja dla DDL: TAK — BEGIN IMMEDIATE → COMMIT.
Strategia locka: TAK — krótkie DDL, UPDATE na końcu, < 1s.
Rollback: TAK — DROP TABLE + backfill rollback + dokumentacja ograniczenia SQLite.
Walidacja po migracji: TAK — 4 kroki.

### Brakujące testy

Brak. Wszystkie findingi CRITICAL i HIGH mają dedykowane testy T1-T25.

### Ryzyka

R1-R9: Bez zmian, wszystkie z mitigacją. Żadne nie jest blokujące.

### Warunek akceptacji

Specyfikacja P1-C zatwierdzona. Wszystkie findingi CRITICAL (F2, F4) i HIGH (F3, F6, F10) rozwiązane. Findingi MEDIUM poprawnie przeniesione do open issues P2 z przypisanymi fazami.

---

## Open issues P2

O1: Mock transport i budżet w testach — flaga gemini_mock_budget_bypass omijająca inkrementację budżetu. Faza P2-A.
O2: Dokładna definicja "artefakty pochodne" w resecie — konkretny SQL DELETE/UPDATE dla każdej tabeli (article_draft_versions, quality_check_runs, generation_operations, article_images, narrative_plans, supplemental_topics). Faza P2-G.
O3: NarrativePlan i generation_batch_items.stage — czy nowy stage w batch item czy pod-etap research/draft. Decyzja wpływa na strukturę dispatch loop. Faza P2-B.
O4: Kalibracja thresholdu bramki semantycznej — domyślny 60, walidacja na fixture'ach i rzeczywistych kandydatach. Faza P2-D.
