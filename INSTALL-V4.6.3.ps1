...
$backup = Join-Path $ProjectRoot "_agent-backups\pre-v4.6.3-$stamp"

Write-Host "Mamona V4.6.3 Tri-Tier Parallel / Delegation Ceiling" -ForegroundColor Cyan
...
$backup | Out-File -Encoding utf8 (Join-Path $ProjectRoot ".kilo\V4.6.3-LAST-BACKUP.txt")

Write-Host ""
Write-Host "V4.6.3 installed." -ForegroundColor Green
Write-Host "Source code, CURRENT_WORK and checkpoints were NOT overwritten by installer."
Write-Host "Next:"
Write-Host "1. powershell -ExecutionPolicy Bypass -File .\VERIFY-V4.6.3.ps1 -ProjectRoot `"$ProjectRoot`""
Write-Host "2. Ctrl+Shift+P -> Developer: Reload Window"
Write-Host "3. New Kilo session -> mamona-coordinator"
Write-Host "4. Paste PROMPT_START_MAMONA_V4.6.3.md"
