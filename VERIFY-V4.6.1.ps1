...
if (Test-Path $coord) {
  ...
  if ($ct -notmatch '"mamona-executor": allow') { Bad "coordinator task allowlist incomplete" } else { Good "task allowlist present" }
}
...
Write-Host "VERIFY PASS — MAMONA V4.6.1 READY" -ForegroundColor Green
exit 0
