...
if ($method === 'GET') {
    $getAction = (string) ($_GET['action'] ?? 'status');
    if ($getAction === 'workflow_status') {
        $rawIds = $_GET['topic_ids'] ?? [];
        if (is_string($rawIds)) $rawIds = array_values(array_filter(explode(',', $rawIds), static fn (string $id): bool => trim($id) !== ''));
        content_studio_json(['ok' => true, 'topics' => generation_workflow_statuses($rawIds)]);
    }
    if ($getAction !== 'status') {
    ...
    content_studio_json([
        ...
        'workflow_batch_limit' => (int) app_config('batch_max_topics'),
...
try {
    ...
    if (in_array($action, ['workflow_action', 'topic_workflow', 'run_topic_workflow'], true)) {
        $requestKey = trim((string) ($_POST['request_key'] ?? ($_SERVER['HTTP_IDEMPOTENCY_KEY'] ?? ''))) ?: null;
        $batch = create_topic_workflow_batch(
            $_POST['topic_ids'] ?? null,
            trim((string) ($_POST['workflow_action'] ?? $_POST['step'] ?? '')),
            $requestKey,
            'admin',
            trim((string) ($_POST['retry_stage'] ?? '')) ?: null
        );
        if (!$batch['terminal']) generation_batch_launch_worker();
        content_studio_json(['ok' => true, 'batch' => $batch, 'outcomes' => array_map(
            static fn (array $item): array => ['topic_id' => (int) $item['topic_id'], 'outcome' => (string) ($item['outcome'] ?? $item['status']), 'status' => (string) $item['status'], 'wait_reason' => (string) $item['wait_reason'], 'error' => (string) $item['error_message']],
            $batch['items']
        )], $batch['terminal'] ? 200 : 202);
    }
